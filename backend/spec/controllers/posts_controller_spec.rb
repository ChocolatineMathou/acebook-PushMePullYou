...
RSpec.describe PostsController, type: :controller do
  ...
  describe "POST /" do
    it "returns json data" do
      post :create, params: { post: { message: "this is a test", customer_id: @customer.id } }
      expect(response).to be_successful
      expect(response.content_type).to eq("application/json")
    end

    it "adds a post to the table" do
      expect { post :create, params: { post: { message: "this is a test", customer_id: @customer.id } } }.to change { Post.all.length }.by(1)
    end
  ...
  describe "GET /" do
    before do
      post :create, params: { post: { message: "this is a test",
                                      customer_id: @customer.id } }
      post :create, params: { post: { message: "this is a second test",
                                      customer_id: @customer.id } }
